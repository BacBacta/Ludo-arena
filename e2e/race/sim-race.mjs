...
t.check('the banner prize is the FIXED prize (3000c), not the faucet budget', A.hello.race?.prizePoolCents === 3000, `prizePoolCents=${A.hello.race?.prizePoolCents}`);
...
await probe.open();
t.check('prize stays FIXED after grants were paid out (display decoupled from faucet spend)', probe.hello.race?.prizePoolCents === 3000, `prizePoolCents=${probe.hello.race?.prizePoolCents}`);
probe.close();
