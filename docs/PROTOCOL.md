...
| `race.board` | `{ top, myRank, myPoints }` | Race Week leaderboard reply. `top` = `[{ name, points, rank }]` (highest first, 1-indexed rank). `myRank`/`myPoints` locate the caller (rank 0 = unranked). A staked game between two event participants scores (win +3, participation +1). Anti wash-trading caps default OFF (event optimises for raw on-chain volume); `RACE_MAX_VS_SAME_PER_DAY` / `RACE_MAX_SCORED_PER_DAY` > 0 re-arm them. |
