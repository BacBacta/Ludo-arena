...
| `race.claim` | `{ passTxHash }` | Race Week onboarding faucet. `passTxHash` = the tx in which the player minted their soulbound `RacePass`. The server (dormant unless `RACE_WEEK_ACTIVE=true` + a funded faucet key + a deployed RacePass) verifies that tx emitted `Minted(myWallet)` from the RacePass, then transfers a tiny event stake budget to the wallet. Anti-sybil: proven wallet required, ONE grant per wallet (the Pass is soulbound → 1/wallet → 1/phone) AND per device fingerprint, plus a hard total-pool cap (`RACE_POOL_CENTS`). Idempotent. Reply `race.claimed`. **Funding mode:** by default the whole per-wallet quota (`RACE_QUOTA_CENTS`) is granted at claim. With `RACE_JIT_FUNDING=true` (mainnet anti-"claim-and-run"), the claim grants only one stake + gas buffer (`RACE_PER_GAME_CENTS`, default 2¢); the rest of the quota is topped up one stake at a time after each COMPLETED event game (server-side, no client message), so a wallet that claims and never plays keeps only that first grant instead of the whole quota. |
...
| `race.claimed` | `{ fundedCents, alreadyFunded, txHash? }` | Ack for `race.claim`: `fundedCents` was just sent to the wallet (`txHash` = the funding transfer), or `alreadyFunded: true` with `fundedCents: 0` on an idempotent re-claim (the wallet already got its one-time grant). A `race.claim` landing inside the 3 s anti-spam window replies `error LIMIT_REACHED` (never a silent drop). |
| `race.seeded` | `{ seedCents, alreadySeeded, txHash?, rateLimited? }` | Ack for `race.seed` (B1 gas seed): `seedCents` of cUSD was sent to cover the burner's mint/join gas; `alreadySeeded: true` + 0¢ when the wallet already holds the target. `rateLimited: true` + 0¢: the request hit the 3 s anti-spam window and nothing was sent — retry shortly. The server ALWAYS replies to `race.seed`; a silent drop left honest clients hanging to their timeout and surfacing "Gas seed failed" when nothing had failed. |
