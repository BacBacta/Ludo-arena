...
| `race.board` | `{ top, myRank, myPoints }` | Race Week leaderboard reply. `top` = `[{ name, points, rank }]` (highest first, 1-indexed rank). `myRank`/`myPoints` locate the caller (rank 0 = unranked). A staked game between two event participants scores (win +3, participation +1), anti-wash-traded (capped per opponent + per day). |
