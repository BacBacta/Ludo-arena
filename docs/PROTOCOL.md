...
| `collection.claimed` | `{ setId, tickets, claimedSets }` | Ack for `collection.claim`: the set bonus was granted (or already had been). `tickets` = new balance; `claimedSets` = the player's full claimed list. Also surfaced in `hello.ok` (`claimedSets`). |
