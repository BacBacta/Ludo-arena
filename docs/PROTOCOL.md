...
| `friends.update` | `{ friends, requests, outgoing? }` | Live refresh of the `FriendInfo` lists, pushed to a player whose graph changed while they had an active session. `outgoing` = requests I SENT that await the other side (the sender's pending-invitations view; withdrawing one is a normal `friend.remove`). Also included in `hello.ok` (`friends`/`friendRequests`/`friendsOutgoing`, walletProven sessions only) with an `online` presence SNAPSHOT. |
...
| `match.found` | `{ gameId, seat, opponent: { name, elo, flag, pid?, avatar?, frame?, tokenSkin?, entranceFx?, victoryFx? }, stakeCents, potCents, fairnessCommit }` | Match found. `fairnessCommit` = hash of the server seed, to display. `opponent` carries the other player's equipped cosmetics: `tokenSkin` patterns THEIR pawns on my board, `entranceFx` plays their entrance burst at match start (cosmetics phase 1), `victoryFx` is the flourish MY end screen plays if THEY win (phase 2). Board themes are local-view-only and never relayed. |
